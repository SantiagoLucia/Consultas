...
FROM (
    ...
    RIGHT JOIN EE_GED.EE_EXPEDIENTE_ELECTRONICO EE ON EE.ID = HIS.ID_EXPEDIENTE AND EE.ESTADO <> 'Guarda Temporal' --AND EE.ID_TRATA IN ()
    ...
    LEFT JOIN EE_GED.EE_EXPEDIENTE_ELECTRONICO EE ON EE.ID = TEMP.ID_EXPEDIENTE 
    ...
    LEFT OUTER JOIN TRACK_GED.SADE_SECTOR_USUARIO SU ON T.ASSIGNEE_=SU.NOMBRE_USUARIO AND SU.ID_SECTOR_USUARIO = 
        (
...
WHERE
    MINFIN.CODIGO_REPARTICION = 'MSGP'
    ORDER BY EE.ID ASC