select  
    ...
    coalesce(regexp_substr(p.groupid_, '[A-Z0-9#]+', 1, 2),
        ssi.codigo_sector_interno) as sector_actual,
...
from 
    ...
    -- caratulacion
    left join track_ged.sade_reparticion rep_caratula 
        on ee.codigo_reparticion_usuario = rep_caratula.codigo_reparticion
    left join track_ged.sade_reparticion min_caratula 
        on rep_caratula.ministerio = min_caratula.id_reparticion
        
    -- esta asignado a usuario
    left join track_ged.sade_sector_usuario su on regexp_substr(t.assignee_, '[^.]+') 
        = su.nombre_usuario and su.estado_registro = 1
    ...
    left join track_ged.sade_reparticion repa 
        on regexp_substr(p.groupid_, '[^-]+') = repa.codigo_reparticion
    left join track_ged.sade_reparticion ministerio 
        on ministerio.id_reparticion = repa.ministerio

    -- reparticion/ministerio actual
    left join track_ged.sade_reparticion repfin 
        on repfin.codigo_reparticion = coalesce(sr.codigo_reparticion, repa.codigo_reparticion)
    left join track_ged.sade_reparticion minfin 
        on minfin.codigo_reparticion = coalesce(sr1.codigo_reparticion, ministerio.codigo_reparticion)
